...
export class LeaveRepository {
  ...
  async cancelLeave(orgId: string, id: string) {
    const r = await queryOne(`UPDATE leave_requests SET status = 'CANCELLED', updated_at = NOW() WHERE organization_id = $1 AND id = $2 RETURNING *`, [orgId, id]);
    return r;
  }
  ...
  async approveLeave(orgId: string, id: string, reviewerId: string, reviewReason: string) {
    ...
    try {
      ...
      if (!balance) {
        const lt = await client.queryOne(`SELECT * FROM leave_types WHERE organization_id = $1 AND id = $2`, [orgId, req.leave_type_id]);
        balance = await client.queryOne(`
          INSERT INTO leave_balances (organization_id, employee_id, leave_type_id, year, total_quota, used, pending, available)
          VALUES ($1, $2, $3, $4, $5, 0, 0, $5)
          RETURNING *
        `, [orgId, req.employee_id, req.leave_type_id, year, lt ? lt.annual_quota : 12]);
      }
