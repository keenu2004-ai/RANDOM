...
export class HolidayShiftRepository {
  ...
  async createHoliday(orgId: string, data: any) {
    const branchId = data.branchId === 'ALL' ? null : data.branchId;

    const dupCheck = await query(`
      SELECT id FROM holidays 
      WHERE organization_id = $1 AND date = $2 AND (branch_id = $3 OR (branch_id IS NULL AND $3 IS NULL))
    `, [orgId, data.date, branchId]);

    if (dupCheck.length > 0) {
      throw new Error('A holiday for this date already exists for this organization/branch.');
    }
    ...
    const res = await queryOne(`
      ...
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `, [orgId, data.name, data.date, data.type, branchId, data.description]);

    const branch = branchId ? await queryOne(`SELECT name FROM branches WHERE organization_id = $1 AND id = $2`, [orgId, branchId]) : null;
  ...
  async updateHoliday(orgId: string, id: string, data: any) {
    ...
    if (data.branchId !== undefined) { fields.push(`branch_id = $${idx++}`); values.push(branchId); }
  ...
  async createShift(orgId: string, data: any) {
    ...
    const r = await queryOne(`
      INSERT INTO shifts (organization_id, name, start_time, end_time, grace_period_minutes, break_duration_minutes, working_hours, week_offs, active)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
      RETURNING *
    `, [orgId, data.name, data.startTime, data.endTime, data.gracePeriodMinutes, data.breakDurationMinutes, data.workingHours, weekOffsStr, data.active]);
  ...
  async updateShift(orgId: string, id: string, data: any) {
    const fields = [];
    const values = [];
    let idx = 1;
  ...
  async assignShiftSingle(orgId: string, employeeId: string, shiftId: string, reason: string, assignedBy: string, assignedByName: string) {
  ...
  async assignShiftBulk(orgId: string, employees: any[], shift: any, reason: string, assignedBy: string, assignedByName: string) {
    const client = await beginTransaction();
    try {
