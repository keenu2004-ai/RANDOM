...
-- Rename helpdesk_messages to ticket_comments if it hasn't been renamed yet
ALTER TABLE IF EXISTS helpdesk_messages RENAME TO ticket_comments;

-- Alter ticket_comments
ALTER TABLE ticket_comments RENAME COLUMN sender_id TO author_employee_id;
ALTER TABLE ticket_comments ADD COLUMN IF NOT EXISTS organization_id UUID REFERENCES organizations(id) ON DELETE CASCADE;
ALTER TABLE ticket_comments ADD COLUMN IF NOT EXISTS attachment_name VARCHAR(255);
ALTER TABLE ticket_comments ADD COLUMN IF NOT EXISTS attachment_url TEXT;
ALTER TABLE ticket_comments ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP;
ALTER TABLE ticket_comments ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE;

-- Backfill organization_id for ticket_comments
UPDATE ticket_comments tc
SET organization_id = ht.organization_id
FROM helpdesk_tickets ht
WHERE tc.ticket_id = ht.id
  AND tc.organization_id IS NULL;

-- Enforce NOT NULL on organization_id for ticket_comments
